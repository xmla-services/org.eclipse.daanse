...
public interface Node extends Comparable<Node> {
  ...
  /**
   * @param n The Node to set as the parent. Mostly used internally. The various
   *          addChild or appendChild sorts of methods should use this to set the
   *          node's parent.
   */
  ...
  /**
   * inserts a child Node at a specific index, displacing the nodes after the
   * index by 1.
   * 
  ...
  /**
   * Replaces a child node with another one. It does nothing if the first
   * parameter is not actually a child node.
   * 
  ...
  /**
   * Insert a Node right before a given Node. It does nothing if the where Node is
   * not actually a child node.
   * 
  ...
  /**
   * Insert a node right after a given Node. It does nothing if the where node is
   * not actually a child node.
   * 
  ...
  /**
   * @return the index of the child Node. Or -1 if it is not a child Node.
  ...
  /**
   * @return a List containing this node's child nodes The default implementation
   *         returns a copy, so modifying the list that is returned has no effect
   *         on this object. Most implementations of this should similarly return
   *         a copy or possibly immutable wrapper around the list.
   */
  ...
  /**
   * @return All the tokens in the node that are "real" (i.e. participate in
   *         parsing)
   */
  ...
  /**
   * @return the #mdxLexer from which this Node object originated. There is no
   *         guarantee that this doesn't return null. Most likely that would
   *         simply be because you constructed the Node yourself, i.e. it didn't
   *         really come about via the parsing/tokenizing machinery.
   */
  mdxLexer getTokenSource();

  void setTokenSource(mdxLexer tokenSource);

  /**
   * @return the original source content this Node came from a reference to the
   *         #mdxLexer that stores the source code and the start/end location info
   *         stored in the Node object itself. This method could throw a
   *         NullPointerException if #getTokenSource returns null. Also, the
   *         return value could be spurious if the content of the source file was
   *         changed meanwhile. But this is just the default implementation of an
   *         API and it does not address this problem!
   */
  ...
  /**
   * @return the offset in the input source where the token begins, expressed in
   *         code units.
   */
  int getBeginOffset();

  /**
   * @return the offset in the input source where the token ends, expressed in
   *         code units. This is actually the offset where the very next token
   *         would begin.
   */
  int getEndOffset();
  ...
  /**
   * Set the offset where the token ends, actually the location where the very
   * next token should begin.
   */
  void setEndOffset(int endOffet);

  /**
   * @return a String that gives the starting location of this Node. This is a
  ...
  /**
   * @return whether this Node was created by regular operations of the parsing
   *         machinery.
   */
  ...
  /**
   * Mark whether this Node is unparsed, i.e. <i>not</i> the result of normal
   * parsing
   * 
  ...
  default Token firstDescendantOfType(mdxConstants.TokenType type) {
    for (int i = 0; i < getChildCount(); i++) {
      Node child = getChild(i);
      if (child instanceof Token) {
        Token tok = (Token) child;
  ...
  default Token firstChildOfType(mdxConstants.TokenType tokenType) {
    for (int i = 0; i < getChildCount(); i++) {
      Node child = getChild(i);
      if (child instanceof Token) {
        Token tok = (Token) child;
  ...
  default mdxConstants.TokenType getTokenType() {
    return this instanceof Token ? ((Token) this).getType() : null;
  }

  /**
   * @return the very first token that is part of this node. It may be an unparsed
   *         (i.e. special) token.
   */
  ...
  default void dump(String prefix) {
    ...
    if (this instanceof Token) {
      ...
      output = String.format("<%s (%d, %d)-(%d-%d)>", getClass().getSimpleName(), getBeginLine(), getBeginColumn(),
  ...
  static abstract public class Visitor {
    ...
    static {
      try {
        // Use this just to represent no method found, since ConcurrentHashMap cannot
        // contains nulls
      ...
      mapLookup = Collections
          .synchronizedMap(new HashMap<Class<? extends Node.Visitor>, Map<Class<? extends Node>, Method>>());
    }
    ...
    /**
     * Tries to invoke (via reflection) the appropriate visit(...) method defined in
     * a subclass. If there is none, it just calls the recurse() routine.
     * 
