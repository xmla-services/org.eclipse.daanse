...
//		Thread.sleep(1000000);
	...
	void testRequest_DISCOVER_PROPERTIES_LocaleIdentifier(@InjectService XmlaService xmlaService) throws Exception {
		...
		SOAPUtil.callSoapWebService(soapEndpointUrl, SOAP_ACTION_DISCOVER, envelop(REQUEST_DISCOVER_PROPERTIES_LocaleIdentifier));

		verify(xmlaService, (times(1))).discover(dicoverCaptor.capture());
	...
	void testRequest_MDSCHEMA_CUBES_Content_Data(@InjectService XmlaService xmlaService) throws Exception {
		...
		SOAPUtil.callSoapWebService(soapEndpointUrl, SOAP_ACTION_DISCOVER, envelop(REQUEST_DISCOVER_MDSCHEMACUBES_CONTENT));
